...
class WalleeCheckoutPaymentModulesThemeContentView extends WalleeCheckoutPaymentModulesThemeContentView_parent
{
    ...
    {
        $key = 'configuration/MODULE_PAYMENT_WALLEE_STATUS';
        $query = xtc_db_query("SELECT `value` FROM `gx_configurations` WHERE `key` = '" . xtc_db_input($key) . "'");
        $enabled = xtc_db_fetch_array($query);
